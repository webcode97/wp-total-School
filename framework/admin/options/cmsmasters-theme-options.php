...
total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-general.php', true);
total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-post.php', true);
total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-page.php', true);
total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-other.php', true);


if (class_exists('Cmsmasters_Projects')) {
	total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-project.php', true);
}

if (class_exists('Cmsmasters_Profiles')) {
	total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-profile.php', true);
}


if (CMSMASTERS_TIMETABLE) {
	total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-tt-event.php', true);
}


if (CMSMASTERS_DONATIONS) {
	total_school_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-campaign.php', true);
	total_school_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-donation.php', true);
}


total_school_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-product.php', true);


if (!function_exists('total_school_get_custom_all_meta_fields')) {
function total_school_get_custom_all_meta_fields() {
	$custom_meta_fields = total_school_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = total_school_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = total_school_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = total_school_get_custom_profile_meta_fields();
		...
		$custom_product_meta_fields = total_school_get_custom_product_meta_fields();
		...
		$custom_campaign_meta_fields = total_school_get_custom_campaign_meta_fields();
		...
		$custom_donation_meta_fields = total_school_get_custom_donation_meta_fields();
		...
		$custom_tt_event_meta_fields = total_school_get_custom_tt_event_meta_fields();
		...
		$custom_post_meta_fields = total_school_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = total_school_get_custom_other_meta_fields();
...
function total_school_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_localize_script('cmsmasters_theme_options_js', 'cmsmasters_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'total-school'), 
			'select_format' => 		esc_attr__('Please select the format.', 'total-school'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'total-school'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'total-school'), 
...
add_action('admin_enqueue_scripts', 'total_school_admin_enqueue_scripts');


function total_school_show_meta_box() {
	global $post;
	
	$custom_all_meta_fields = total_school_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'total-school') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'total-school') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'total-school') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'total-school') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'total-school') . '</option>';
		...
		case 'image':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
				'<input type="button" id="cmsmasters_upload_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'total-school') . '" data-title="' . esc_attr__('Choose Image', 'total-school') . '" data-button="' . esc_attr__('Insert Image', 'total-school') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] === 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $field['multiple'] . '" />' . 
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'total-school') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'total-school') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'total-school') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'total-school') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'total-school') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'total-school') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'total-school') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'total-school') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'total-school') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'total-school') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'total-school') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'total-school') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'total-school') : esc_attr__('Create Gallery', 'total-school')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'total-school') . '" data-button="' . esc_attr__('Insert Gallery', 'total-school') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'total-school') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'total-school') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'total-school') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function total_school_save_custom_meta($post_id) {
    $custom_all_meta_fields = total_school_get_custom_all_meta_fields();
...
add_action('save_post', 'total_school_save_custom_meta');


function total_school_add_custom_meta_box() {
	$args = array(
	   'public' => true 
	);
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'total-school'), 
			'total_school_show_meta_box', 
			$screen, 
			'normal', 
			'high' 
		);
...
add_action('add_meta_boxes', 'total_school_add_custom_meta_box');
