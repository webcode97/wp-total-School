...
function total_school_get_admin_page() {
	global $pagenow;
...
function total_school_default_tab() {
	$current_page = total_school_get_admin_page();
...
function total_school_get_the_tab() {
	global $pagenow;
	
	$default_tab = total_school_default_tab();
...
function total_school_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'total-school') . '</a>';
		}
